...
export class RequestService {
    ...
    public async request<T>(method: 'GET' | 'PUT' | 'POST' | 'DELETE', url: string, body?: unknown): Promise<HttpResponse<T>> {
        const response$ = this.requestWithObservable<T>(method, url, body);
        return firstValueFrom(response$);
    }

    public requestWithObservable<T>(method: 'GET' | 'PUT' | 'POST' | 'DELETE', url: string, body?: unknown): Observable<HttpResponse<T>> {
        ...
        let request = this.httpClient.request<T>(method, url, { body: bodyToSend, headers, observe: 'response' });

        request = request.pipe(catchError(errorResponse => {
